...
const DetailBook = ({ book: initialBook, onSearchByAuthor }) => {
  ...
  useEffect(() => {
    if (initialBook) { // Đảm bảo initialBook có giá trị trước khi fetch
  ...
  const fetchAuthor = async (authorId) => { // Nhận authorId làm tham số
  ...
  const fetchSimilarBook = async (currentBookId) => { // Nhận currentBookId
    if(!currentBookId) return;
    setSimilarBooks([]);
    try {
      const response = await fetch(`${BASE_URL}/books/api/random/${currentBookId}`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();
  ...
  const handleBorrowBook = async () => {
    try {
      ...
      if (response.status === 201 && response.data) {
        toast.success("Yêu cầu mượn sách đã được gửi! Chúng tôi sẽ sớm liên hệ với bạn.", {
        ...
        try {
            ...
            console.error("Lỗi khi gửi thông báo:", notificationError.response?.data || notificationError.message);
        }
        
      } else {
        console.warn("Trạng thái phản hồi không mong muốn:", response.status, "Dữ liệu:", response.data);
      ...
      if (error.response) {
        console.error("Dữ liệu lỗi phản hồi khi mượn sách:", error.response.data);
  ...
  useEffect(() => {
    if (initialBook && initialBook.id) { // Kiểm tra initialBook và initialBook.id
        setBook(initialBook);
  ...
  if (!book || !book.id) {
    ...
    return (
        <Container className="mt-3 mb-5 book-detail-container">
            <Alert variant="warning">Đang tải thông tin sách hoặc sách không tồn tại...</Alert>
        </Container>
    );
  }
  const renderRatingStars = (rating) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(<FontAwesomeIcon key={`full-${i}`} icon={faSolidStar} className="text-warning" />);
    }

    if (hasHalfStar && stars.length < 5) { // Chỉ thêm nửa sao nếu chưa đủ 5 sao
      ...
      stars.push(<FontAwesomeIcon key="half" icon={faRegularStar} className="text-warning" />);
    }
         ...
         <Col lg={8}>
             ...
             <Card.Body>
               <Row>
                 <Col md={4} className="mb-4 mb-md-0">
                   <Image
                     src={book.image && book.image.startsWith('http') ? book.image : `${BASE_URL}${book.image}`} // Sửa đường dẫn ảnh
                     alt={book.title}
                     fluid
                     className="shadow-sm rounded book-detail-image"
                     onError={(e) => { e.target.onerror = null; e.target.src="/icon.png"; }} // Fallback image
                   />
                 ...
                 <Col md={8}>
                   <div className="d-flex justify-content-between align-items-start">
                     <div>
                       ...
                       <h2 className="h5 text-muted mb-3">
                         bởi {book.author?.name || 'N/A'} ({book.publication_date}) {/* Thêm optional chaining */}
                       </h2>
                     </div>
                     <Button
                       variant="link"
                   ...
                   <div className="mb-3">
                     ...
                     <span className="text-muted">{book.rating?.toFixed(1) || 'N/A'}</span>
                   </div>
                   <div className="d-flex flex-wrap gap-2 mb-3">
                     {book.category?.map((obj, index) => ( // Thêm optional chaining
                   ...
                   >
                     ...
                     <ProgressBar
                       now={(book.quantity > 0 ? (book.avaliable / book.quantity) : 0) * 100} // Tránh chia cho 0
                       variant={book.avaliable > 0 ? "success" : "danger"}
           ...
           <Tabs defaultActiveKey="description" id="book-tabs" className="mb-4">
             <Tab eventKey="description" title="Mô tả">
               <Card>
                 <Card.Body>
                   <p className="lead" style={{ whiteSpace: "pre-line" }}>{book.description}</p>
                 </Card.Body>
               </Card>
             </Tab>
           </Tabs>
           ...
           <Card className="mb-4">
             ...
             <Card.Body>
               <div className="d-flex mb-3">
                 <Image
                   src={author?.avatar && author.avatar.startsWith('http') ? author.avatar : (author?.avatar ? `${BASE_URL}${author.avatar}` : "/icon.png")} // Sửa đường dẫn ảnh
                   roundedCircle
                   width={80}
                   height={80}
                 ...
                 {loadingAuthor ? (
                   ...
                   <Alert variant="danger" className="p-2 small">{errorAuthor}</Alert>
                 ) : author ? (
                   <div>
                     <h5 className="mb-1">{author.name}</h5>
                     <p className="text-muted small">{author.jobs || "Chưa có thông tin nghề nghiệp"}</p>
                   </div>
                 ) : (
                    <p>Không có thông tin tác giả.</p>
                 )}
               </div>
               <p className="author-biography" style={{ whiteSpace: "pre-line" }}>{author?.biography ? author.biography : ""}</p>
               {author && book.author && ( // Chỉ hiển thị nút khi có thông tin tác giả
                <Button
                    variant="outline-primary"
                    size="sm"
             ...
             <Card.Body>
               {similarBooks.length > 0 ? (
                <ListGroup variant="flush">
                    ...
                    <ListGroup.Item key={index} className="border-0 p-2 similar-book-item">
                        <div className="d-flex">
                        <Image 
                            src={obj.image && obj.image.startsWith('http') ? obj.image : `${BASE_URL}${obj.image}`}
                            width={60} 
                            height={80}
                        ...
                        <div>
                            ...
                            <p className="small text-muted mb-0">bởi {obj.author?.name || 'N/A'}</p>
                            <div className="small text-warning">{renderRatingStars(obj.rating || 0)}</div>
                        </div>
                        </div>
                    </ListGroup.Item>
                    ))}
