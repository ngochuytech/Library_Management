...
const PersonalLibrary = ({ userId }) => {
    ...
    const fetchBorrows = async () => {
      try {
        ...
        const processedData = rawData.map((borrow) => {
          ...
          if (expDate && (String(borrow.status).toLowerCase() === "approved" || String(borrow.status).toLowerCase() === "borrowed")) {
        ...
        toast.error(error.response?.data?.detail || error.message || "Không tải được lịch sử mượn sách.");
        setBorrows([]);
      } finally {
        setLoading(false);
      }
  ...
  const calculateRemainingDays = (expDateStr, borrowStatus) => {
    ...
    const startOfExpDate = new Date(expDate.getFullYear(), expDate.getMonth(), expDate.getDate());
  ...
  const handleActionClick = (borrow, action) => {
    ...
    if (action === "Hủy YC" && borrow.status === "PENDING") {
        ...
        // axios.post(`${BASE_URL}/borrows/api/cancel/${borrow.id}`, {}, { headers: { Authorization: `Bearer ${sessionStorage.getItem("access_token")}` }})
        // .then(() => { toast.success("Yêu cầu đã được hủy."); fetchBorrows(); })
  ...
  const getBorrowTransactionStatusBadge = (borrowStatus, isOverdue) => {
    ...
    if (isOverdue && (statusLower === "borrowed" || statusLower === "approved")) {
      variant = "danger";
      label = "Quá hạn";
    ...
    return (
      <Badge pill bg={variant} text={variant === "light" || variant === "info" ? "dark" : "white"} className="px-2 py-1">
  ...
  const getActionButton = (borrow) => {
    ...
    if (statusLower === "pending") {
        config = { label: "Hủy YC", variant: "outline-danger", icon: faHandPointer, disabled: false };
    } else if (borrow.isOverdue) {
        if (statusLower === "borrowed") {
            pass
        } else if (statusLower === "approved") {
            config = { label: "YC hết hạn", variant: "outline-danger", icon: faTimesCircle, disabled: true };
        }
        // Các trường hợp isOverdue khác sẽ không có nút nếu không được định nghĩa ở trên (config vẫn là null)
    } else { // Không quá hạn
        if (statusLower === "borrowed") {
            config = { label: "Trả sách", variant: "outline-primary", icon: faUndo, disabled: false };
        } else if (statusLower === "approved") {
  ...
  const calculateDueProgress = (expDateStr, borrowStatus, actualBorrowDateStr) => {
    const statusLower = String(borrowStatus).toLowerCase();
    if (!["borrowed", "approved"].includes(statusLower) || !expDateStr ) return 0;

    const now = new Date();
    const due = new Date(expDateStr);
    const start = actualBorrowDateStr ? new Date(actualBorrowDateStr) : new Date(due.getTime() - (7 * 24 * 60 * 60 * 1000)); 

    if (isNaN(start.getTime()) || isNaN(due.getTime()) || start >= due) return 0;
  ...
  const filteredBorrows = borrows.filter((borrow) => {  
    ...
    if (activeTab === "all") {
      ...
      if ((statusLower === "approved" || statusLower === "borrowed") && !borrow.isOverdue) {
        tabMatch = true;
      }
    } else if (activeTab === "overdue") {
      if ((statusLower === "approved" || statusLower === "borrowed") && borrow.isOverdue) tabMatch = true;
    } else if (activeTab === "waiting") {
      if (statusLower === "pending") tabMatch = true; 
    } else if (activeTab === "history") {
      if (["returned", "canceled", "approved_return"].includes(statusLower)) tabMatch = true;
    }
    if (!tabMatch) return false;
    
    const isBorrowingOrApproved = (statusLower === "approved" || statusLower === "borrowed");

    if (filterDays !== "all") {
        if (filterDays === "0") { 
            if (!(isBorrowingOrApproved && (borrow.isOverdue || borrow.remainingDays === 0))) return false;
        } else if (isBorrowingOrApproved && !borrow.isOverdue && borrow.remainingDays !== null) {
            if (filterDays === "7") { 
                if (!(borrow.remainingDays >= 7)) return false;
            } else if (filterDays === "3") { 
                if (!(borrow.remainingDays >= 3 && borrow.remainingDays < 7)) return false;
            }
          ...
          <Nav variant="tabs" className="flex-grow-1 mb-2 mb-md-0">
            ...
            <Nav.Item>
              <Nav.Link eventKey="current">
                ...
                Đang mượn ({borrows.filter(b => (String(b.status).toLowerCase() === "approved" || String(b.status).toLowerCase() === "borrowed") && !b.isOverdue).length})
              </Nav.Link>
            </Nav.Item>
            <Nav.Item>
              <Nav.Link eventKey="overdue">
                <FontAwesomeIcon icon={faExclamationTriangle} className="me-1 text-danger" />
                Quá hạn ({borrows.filter(b => (String(b.status).toLowerCase() === "approved" || String(b.status).toLowerCase() === "borrowed") && b.isOverdue).length})
              </Nav.Link>
            </Nav.Item>
            <Nav.Item>
                <Nav.Link eventKey="waiting">
                    <FontAwesomeIcon icon={faHourglassHalf} className="me-1 text-info" /> 
                    Chờ xử lý ({borrows.filter(b => String(b.status).toLowerCase() === "pending").length})
                </Nav.Link>
            </Nav.Item>
            <Nav.Item>
              <Nav.Link eventKey="history">
                <FontAwesomeIcon icon={faCalendarAlt} className="me-1" />
                Lịch sử ({borrows.filter(b => ["returned", "canceled", "approved_return"].includes(String(b.status).toLowerCase())).length})
              </Nav.Link>
            </Nav.Item>
          </Nav>
      ...
      {filteredBorrows.length === 0 ? (
        ...
        <Row xs={1} md={2} lg={3} xl={4} className="g-4 mt-0">
          {filteredBorrows.map((borrow) => {
            ...
            const isConsideredActive = isActivelyBorrowed || isApprovedForPickup; 
            const isOverdueForDisplay = isConsideredActive && borrow.isOverdue;

            const startDateForProgress = borrow.borrow_date || (isApprovedForPickup ? borrow.require_date : null);
            const dueProgress = calculateDueProgress(borrow.exp_date, borrow.status, startDateForProgress);
            const remainingDaysDisplay = borrow.remainingDays;

            return (
              <Col key={borrow.id || borrow.book_id}> 
                <Card className={`h-100 shadow-sm border-0 overflow-hidden ${isOverdueForDisplay ? 'border-2 border-danger' : ''}`}>
                  <div className="position-relative">
                    <Card.Img
                      ...
                      style={{ height: "200px", objectFit: "cover", filter: isOverdueForDisplay ? "sepia(20%)" : "none" }}
                    />
                    {isOverdueForDisplay && ( 
                  ...
                  <Card.Body className="d-flex flex-column p-3">
                    ...
                    <div className="small text-muted mt-1 mb-2">
                      {borrow.require_date && ( 
                        <div><FontAwesomeIcon icon={faCalendarAlt} className="me-1" />Ngày YC: {new Date(borrow.require_date).toLocaleDateString('vi-VN')}</div>
                      )}
                      {isActivelyBorrowed && borrow.borrow_date && (
                          <div><FontAwesomeIcon icon={faCalendarAlt} className="me-1 text-success" />Mượn từ: {new Date(borrow.borrow_date).toLocaleDateString('vi-VN')}</div>
                      )}
                      {isConsideredActive && borrow.exp_date && (
                        <div><FontAwesomeIcon icon={faClock} className={`me-1 ${isOverdueForDisplay ? 'text-danger' : 'text-secondary'}`} />Hạn trả/lấy: {new Date(borrow.exp_date).toLocaleDateString('vi-VN')}</div>
                      )}
                      {borrow.return_date && ["returned", "approved_return"].includes(statusLower) && (
                    ...
                    <div className="mt-auto pt-2">
                      {(isConsideredActive) && !isOverdueForDisplay && borrow.exp_date && (
                        <>
                            ...
                            <span className={remainingDaysDisplay !== null && remainingDaysDisplay < 3 ? "text-warning fw-bold" : ""}>
                              {remainingDaysDisplay !== null 
                                ? (remainingDaysDisplay >= 0 
                                    ? `${remainingDaysDisplay} ngày còn lại${remainingDaysDisplay === 0 ? " (Hôm nay)" : ""}` 
                                    : `Quá ${Math.abs(remainingDaysDisplay)} ngày`) 
                                : 'N/A'}
                            </span>
                          </div>
                          <ProgressBar
                      ...
                      {isOverdueForDisplay && ( 
                        <Alert variant="danger" className="p-2 small text-center mb-2">
                          <FontAwesomeIcon icon={faExclamationTriangle} className="me-1"/>
                          {statusLower === "approved" ? "Quá hạn lấy sách!" : `Đã quá hạn ${remainingDaysDisplay !== null ? Math.abs(remainingDaysDisplay) : ''} ngày!`}
                        </Alert>
                      )}
